...
int  test_order_w() {  //顺序写
    ...
    for (int i = 0; i < naddrs/2; ++i) {        //first lun
    ...
    for (int i = naddrs/2; i < naddrs; ++i) {            //second lun
        addrs[i].ppa = blk_addr.ppa;
        ...
        addrs[i].g.pl = 0;
        addrs[i].g.blk=block;
        addrs[i].g.pg = ((i-naddrs/2)/geo->nsectors) % geo->npages;
        addrs[i].g.sec = i % geo->nsectors;
    ...
    /*read*/
    for(int luns=0;luns<2;luns++) {
        for (size_t pg = 0; pg < 8; ++pg) {     //first lun
